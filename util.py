...
class Utility:
    ...
    def ReadFile(self, filename: str, extension: str, directory: str = ""):
        ...
        try:
            ...
            log.critical(f"Error al leer {filename}.{extension}, {e}")
...
class ImageUtil:
    ...
    def Font(
        ...
        try:
            ...
            log.warn(
                "BurbankBigCondensed-Black.otf no encontrado, se usara LuckiestGuy-Regular.ttf"
            )

            return ImageFont.truetype(f"{directory}LuckiestGuy-Regular.ttf", size)
        except Exception as e:
            log.error(f"Error al cargar la fuente, {e}")
